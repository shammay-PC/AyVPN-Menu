...
press_any_key() {
    echo ""
    read -p "عملیات با موفقیت انجام شد. برای بازگشت به منو، هر کلیدی را فشار دهید..." -n 1 -r
}
...
# ------------------------------------------------------------------
install_script() {
    echo -e "${YELLOW}به نظر می‌رسد این اولین اجرای اسکریپت است.${RESET}"
    read -p "آیا مایلید دستور '${SCRIPT_NAME}' به عنوان شورتکات دائمی در سیستم نصب شود؟ (y/n): " choice
    if [[ "$choice" == "y" || "$choice" == "Y" ]]; then
        if cp "${CURRENT_SCRIPT_PATH}" "${SCRIPT_PATH}" && chmod +x "${SCRIPT_PATH}"; then
            echo -e "${GREEN}نصب با موفقیت انجام شد! از این پس می‌توانید فقط با اجرای دستور '${SCRIPT_NAME}' منو را باز کنید.${RESET}"
            sleep 3
        else
            echo -e "${RED}خطا در نصب! لطفاً اسکریپت را با دسترسی root اجرا کنید (sudo).${RESET}"
            sleep 3
        fi
    fi
}
...
submenu_v2ray_sub() {
    while true; do
        ...
        echo "------------------------------------"
        echo -e "${WHITE}0)${RESET} بازگشت به منوی قبلی"
        ...
        case $choice in
            ...
            0)
                ...
                ;;
            *)
                handle_error "گزینه نامعتبر است!"
                ;;
        esac
    done
}
...
submenu_nginx() {
    while true; do
        ...
        echo "------------------------------------"
        echo -e "${WHITE}0)${RESET} بازگشت به منوی اصلی"
        read -p "لطفا یک گزینه را انتخاب کنید: " choice
...
exit 0
EOF
                handle_success "فایل /etc/rc.local با موفقیت ایجاد/بازنویسی شد."
                ;;
            2)
...
if [ "$(id -u)" -ne 0 ]; then
   echo -e "${RED}این اسکریپت باید با دسترسی روت (root) اجرا شود. لطفاً از 'sudo' استفاده کنید.${RESET}"
   exit 1
fi
